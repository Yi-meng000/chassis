...
FDCAN_SendQueueType CAN1_Txqueue,CAN1_Rxqueue,CAN2_Txqueue,CAN2_Rxqueue,CAN3_Rxqueue,CAN3_Txqueue;
bool Can1FullFlag = 0;
bool Can2FullFlag = 0;
void CAN_InitSendQueue(void)
{
	...
	CAN3_Rxqueue.Front = CAN3_Rxqueue.Rear = 0;
	...
	for(int i = 0 ; i < FDCAN_QUEUESIZE ; i++)
	{
		...
		CAN3_Rxqueue.FDCAN_DataSend[i].IDE = FDCAN_STANDARD_ID;
		CAN3_Txqueue.FDCAN_DataSend[i].IDE = FDCAN_STANDARD_ID;
	}
	return;
}
...
void CAN_DequeueTx( FDCAN_SendQueueType *queue)
{
	...
	if(queue->Canx == FDCAN1)
		...
		HAL_FDCAN_AddMessageToTxFifoQ(&hfdcan3,&txmessage,Tx_msg);
}
