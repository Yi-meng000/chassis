...
//    }
    for(int i=0;i < USE_DJNUM ;i++)
		{
        PID_Init(&DJmotor[i].posPID,2,0.08,0,PIDINC);
        PID_Init(&DJmotor[i].rpmPID,5.5,0.3,0,PIDINC);
		}
    //PID参数初始化
				PID_Init(&DJmotor[3].posPID,2,0.05,0,PIDINC);
}
...
void DJReceiveData_CAN(FDCAN_RxHeaderTypeDef Rxheader,uint8_t *Rx_data)
{
    //收到的ID只会在201至208内
    //而发出的ID要考虑的就多了 
...
void DJCurrentTransmit(DJMotorPointer motor)
{
    ...
    if(motor->ID == 4 || motor->ID ==8) //积攒到4个或8个再发出，提高传递信息效率
        HAL_FDCAN_AddMessageToTxFifoQ(&hfdcan3,&Txheader,Tx_data);
...
void DJMonitor(DJMotorPointer motor)
{
    if(motor->Begin && motor->Enable){
        ...
        if(motor->error.lastRxTime++ > 50)
            if(motor->error.timeoutCount++ >20)
            {
                motor->error.timeoutCount = 0;
								motor->Enable = false;
								motor->statusflag.Overtimeflag = 1;
